...
│   └── install_jenkins.sh  # now using file() in Terraform
├── terraform/             # Terraform configs
│   ├── provider.tf
│   ├── variables.tf
│   ├── main.tf            # uses file() instead of templatefile()
│   └── outputs.tf
├── Jenkinsfile            # Updated pipeline with placeholders
├── .gitignore
└── README.md              # This file
```
...
Terraform auto-generates an SSH keypair and saves the private key locally. Review or override defaults in `variables.tf` or create `terraform.tfvars`:

```hcl
variable "region" { default = "us-east-1" }
variable "ami_name_filter" { default = "amzn2-ami-hvm-*-gp2" }
variable "instance_type" { default = "t2.large" }
variable "my_ip" { description = "Your IP CIDR for SSH" }
```
...
* Generate RSA keypair → upload public key → save private key as `deployer_key.pem`
* Lookup latest Amazon Linux 2 AMI via `data.aws_ami`
* Create VPC, Subnet, IGW, Route Table, Security Group
* Launch EC2 instance with `install_jenkins.sh` (via `file()`)
...
docker ps  # should show SonarQube
trivy --version  # verifies Trivy binary
```
...
4. Create credentials:
   ...
   * **SonarQube Token**: Secret Text → ID `sonar-server`
...
2. Choose “Pipeline script from SCM” if using `checkout scm`, or select “Pipeline script” and paste the `Jenkinsfile` below.
3. Ensure the `Jenkinsfile` in repo root has your GitHub URL, Docker Hub username, and matches credential IDs.

```groovy
// see Jenkinsfile in repo root — updated with:
// - git URL: https://github.com/<YOUR-ORG>/<YOUR-REPO>.git
// - DOCKER_CREDENTIAL = 'dockerhub-creds'
// - IMAGE_NAME = '<YOUR-DOCKERHUB-USER>/amazon:latest'
```
...
* npm install & test (in `app/`)
* OWASP Dependency‑Check
* Trivy filesystem & image scans
* Docker build & push
* Run App container + smoke test
...
* **Modular Terraform**: separate modules per function
